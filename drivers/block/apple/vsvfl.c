...
static error_t physical_block_to_virtual_block(struct apple_vfl *_vfl, uint32_t _pBlock, uint32_t *_vBank, uint32_t *_vBlock)
{
	...
	_vfl->physical_to_virtual(_vfl, 0, vfl->geometry.pages_per_block * _pBlock, &vBank, &vPage);
...
{
	struct VSVFL *vfl = get_vsvfl(_vfl);
	vfl_checksum(&vfl->contexts[ce], (uint32_t)&vfl->contexts[ce].checksum1 - (uint32_t)&vfl->contexts[ce],
...
static vfl_vsvfl_context_t* get_most_updated_context(struct apple_vfl *vfl) {
	int ce = 0;
	int max = 0;
...
static inline error_t vfl_vsvfl_setup_geometry(apple_vfl *_vfl)
{
	struct VSVFL *vfl = get_vsvfl(_vfl);

#define nand_load(what, where) (vfl->geometry.where = apple_nand_get(_vfl->devices[0], (what)))
	...
	nand_load(NAND_BYTES_PER_PAGE, bytes_per_page);
	...
	vfl->geometry.num_ce = vfl->num_chips;
