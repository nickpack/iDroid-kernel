...
static void iphone_spi_transfer_setup(struct work_struct *_struct)
{
	struct iphone_spi_master *master = container_of(_struct, struct iphone_spi_master, setup_work);
	int speed = (master->transfer->speed_hz <= 0) ? master->message->spi->max_speed_hz : master->transfer->speed_hz;
	unsigned long flags;
	
	if(speed <= 0)
	{
		spi_err(master, "invalid speed %d.\n", speed);
		master->message->status = -EINVAL;
		iphone_spi_message_complete(master);
		return;
	}
	
	spin_lock_irqsave(&master->lock, flags);
	{
		u32 clockFrequency = (master->clock_source == PCLK) ? FREQUENCY_PERIPHERAL : FREQUENCY_FIXED;
		u32 divider;
		u32 options;
		...
		{
			spi_err(master, "divider too high, unable to send (%d).\n", divider);
		}
		else
		{
			struct spi_transfer *tx = master->transfer;
			uint32_t *io_base = master->io_base;
			uint32_t *status = IPHONE_SPI_STATUS(io_base);

			spi_dbg(master, "clock %d (%d) = %d / %d.\n", (clockFrequency/divider), speed, clockFrequency, divider);
			
			writel(divider, IPHONE_SPI_CLKDIV(io_base));

			options = ((master->message->spi->mode & 0x3) << 1)
					| (0x3 << 3)	// Master
					| (1 << 5)		// Interrupt Mode
					| (1 << 8)		// Data Ready interrupt
					| (master->clock_source << IPHONE_SPI_CLOCK_SHIFT);

			if(tx->rx_buf && !tx->tx_buf)
				options |= 1; // Automatic Garbage
			...
			spi_dbg(master, "starting transfer %p, rx=%p, tx=%p, options=0x%08x, txc=%d, rxc=%d.\n",
					master->transfer, tx->rx_buf, tx->tx_buf, options, IPHONE_SPI_TX_BUFFER_USED(status), IPHONE_SPI_RX_BUFFER_USED(status));

			if(tx->tx_buf)
			{
				master->txBuffer = tx->tx_buf;

				if(tx->len > IPHONE_SPI_MAX_TX_BUFFER)
					master->txCurrentLen = IPHONE_SPI_MAX_TX_BUFFER;
				else
					master->txCurrentLen = tx->len;

				master->txTotalLen = tx->len;
				master->txDone = false;
			}
			else
			{
			...
			if(tx->rx_buf)
			{
				master->rxBuffer = tx->rx_buf;
				master->rxCurrentLen = 0;
				master->rxTotalLen = tx->len;
				master->rxDone = false;
			}
			else
			{
			...
			if(tx->rx_buf)
				writel(master->rxTotalLen, IPHONE_SPI_RXAMT(io_base));
			else
			...
			if(tx->tx_buf)
			{
				...
				for(i = 0; i < master->txCurrentLen; i++)
				{
			...
			spi_dbg(master, "transfer set.\n");

			mod_timer(&master->timeout, jiffies + msecs_to_jiffies(IPHONE_SPI_TIMEOUT_MSECS));
		}
	}
...
static int iphone_spi_transfer_start(struct iphone_spi_master *_master, struct spi_transfer *_transfer)
{
	...
	schedule_work(&_master->setup_work);
...
static int __init iphone_spi_probe(struct platform_device *_pdev)
{
	...
	for(i = 0; i < ARRAY_SIZE(iphone_spi_masters); i++)
	{
		...
		INIT_WORK(&master->setup_work, iphone_spi_transfer_setup);
